<script lang="ts">
  ...
  async function handleSocialAuth(providerName: 'google' | 'github') {
    if (loading) return;
    loading = true;
    error = '';
    ...
    <div class="w-full max-w-[480px] mx-auto" in:fly={{ x: 50, duration: 800 }}>
      <div class="card-premium p-8 md:p-10 bg-surface/80 backdrop-blur-xl border-white/5 relative overflow-hidden">
        ...
        <div class="relative z-10">
          ...
          {#if error}
            <div class="p-4 mb-6 rounded-xl bg-danger/10 border border-danger/20 text-danger text-sm font-bold flex items-center gap-3" in:slide>
              <ShieldAlert size={18} />
              {error}
            </div>
          {/if}
          ...
          <form class="space-y-6" onsubmit={handleSubmit}>
            {#if mode === 'signup'}
              <div class="space-y-1.5" in:slide>
                ...
                <div class="relative group">
                  ...
                  <input type="text" id="name" bind:value={name} placeholder="Adaeze Okonkwo" class="form-input pl-12" required />
                </div>
              </div>
            {/if}
            ...
            <div class="space-y-1.5">
              ...
              <div class="relative group">
                ...
                <input type="email" id="email" bind:value={email} placeholder="adaeze@enterprise.ng" class="form-input pl-12" required />
              </div>
            </div>
            ...
            {#if mode !== 'forgot'}
              <div class="space-y-1.5" in:slide>
                ...
                <div class="relative group">
                  ...
                  <input type="password" id="password" bind:value={password} placeholder="••••••••" class="form-input pl-12" required />
                </div>
              </div>
            {/if}

            <button type="submit" class="btn-primary w-full py-4 text-base shadow-xl shadow-emerald/20 group disabled:opacity-50" disabled={loading}>
              {#if loading}
          ...
          <div class="mt-10 pt-8 border-t border-white/5 space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              <button 
                ...
                disabled={loading}
                class="flex items-center justify-center gap-3 py-3 rounded-xl bg-white/5 border border-white/5 text-white hover:bg-white/10 transition-all font-bold text-sm disabled:opacity-50"
              >
            ...
            <p class="text-center text-slate-dim font-medium">
              {#if mode === 'login'}
                Don't have an account? <button type="button" onclick={toggleMode} class="text-emerald font-bold hover:underline">Sign up for free</button>
              {:else if mode === 'signup'}
                Already have an account? <button type="button" onclick={toggleMode} class="text-emerald font-bold hover:underline">Sign in instead</button>
              {:else}
                Remember your password? <button type="button" onclick={() => mode = 'login'} class="text-emerald font-bold hover:underline">Back to login</button>
              {/if}
            </p>
          </div>
        </div>
